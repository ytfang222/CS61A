...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, thing_type, price):
    ...
    def vend(self):
        if self.current_num == 0:
            ...
            if self.balance > 0 :
                print('Here is your {}'.format(self.thing_type) + ' and ${} change.'.format(self.balance))
            else:
                print('Here is your {}.'.format(self.thing_type))
            self.current_num = self.current_num - 1
            self.balance = 0
            
    def deposit(self, add_money):
        if self.current_num == 0:
            print('Machine is out of stock. Here is your ${}.'.format(add_money))
        else :
            self.balance = self.balance + add_money
            print('Current balance:${}'.format(self.balance))
    
    def restock(self,add_num):
        self.current_num = self.current_num + add_num
        print('Current {} stock: {}'.format(self.thing_type,self.current_num))
...
def preorder(t):
    ...
    # b = str(t).replace('Tree(','').replace('[', '').replace(']', '').replace(' ', '').replace(')','').replace('\n','')
    # c = [eval(i) for i in b if i != '']
    # return c
